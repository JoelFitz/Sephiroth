...
TEXTURE2D(_MainTex);
      ...
      MicroSplatLayer Sample(Input i, half4 weights, inout Config config, float camDist, float3 worldNormalVertex, DecalOutput decalOutput)
      {
         ...
         BLEND_GLOBAL_TINT_PERTEX(samples, config, camDist, globalSlopeFilter, noiseUV)
         #endif
         ...
         // Often, as an optimization, you can compute the non-per tex version of above effects here..
            ...
            BLEND_GLOBAL_TINT(albedo.rgb, config, camDist, globalSlopeFilter, noiseUV)
         #endif
...
float3 GetTessFactors ()
{
   ...
   Dependency "BaseMapShader" =  "Hidden/MicroSplat/Main Terrain_Base"
   Fallback "Hidden/MicroSplat/Main Terrain_Base"
